          ...
          <Link to={`/blogs/${post.title.toLowerCase().replace(/\s+/g, '-')}`} key={post._id} className='relative rounded-lg overflow-hidden'>
            ...
            <div className='absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 p-4 text-white'>
              <div className='flex items-center mb-2'>
                {/* Assuming user has a profile picture */}
                {/* <img src={`http://localhost:8080/api/uploads/${post.user.profilePic}`} alt={post.user.username} className='w-8 h-8 rounded-full mr-2' /> */}
                <div>
